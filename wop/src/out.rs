...
#[allow(non_snake_case, clippy::enum_glob_use, unused_braces, unused_imports)]
pub fn reduct_map() -> ReductMap<Meta<Ast>, Sym> {
    ...
    match toks[2].item.item {
        Ast::RulePipe(ref v) => rule_vec.push((v.clone(), toks.get(3).map(|g| g.item.span).unwrap_or(Span::new(0, 0)))),
        _ => unreachable!(),
    };
    Ast::Rule(rule_vec)
}, lrp::Span::new(toks[0].item.span.start, toks.last().unwrap().item.span.end))
    ...
    Ast::Rule(vec![(prod.clone(), toks.get(1).map(|g| g.item.span).unwrap_or(Span::new(0, 0)))])
}, lrp::Span::new(toks[0].item.span.start, toks.last().unwrap().item.span.end))
	}
	...
	fn lrp_wop_RuleItem_0(toks: &[Gramem]) -> lrp::Meta<Ast> {
		lrp::Meta::new( {
    Ast::RuleItem(Box::new(toks[0].clone()), toks.len() > 1)
}, lrp::Span::new(toks[0].item.span.start, toks.last().unwrap().item.span.end))
	}
	...
	fn lrp_wop_RuleItem_4(toks: &[Gramem]) -> lrp::Meta<Ast> {
		lrp::Meta::new( {
    Ast::RuleItem(Box::new(toks[1].clone()), toks.len() > 3)
}, lrp::Span::new(toks[0].item.span.start, toks.last().unwrap().item.span.end))
	}
	map.insert(RuleItem, vec![lrp_wop_RuleItem_0, lrp_wop_RuleItem_1, lrp_wop_RuleItem_2, lrp_wop_RuleItem_3, lrp_wop_RuleItem_4, 	]);
