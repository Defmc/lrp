        ...
        lrp::Meta::new(
            {
                let cb = Some(&toks[3]);
                {
                    ...
                    match toks[2].item.item {
                        Ast::RulePipe(ref v) => rule_vec
                            .push((v.clone(), cb.map_or(|| Span::new(0, 0), |g| g.item.span))),
                        _ => unreachable!(),
                    };
                    Ast::Rule(rule_vec)
                }
        ...
        lrp::Meta::new(
            {
                let cb = Some(&toks[1]);
                {
                    ...
                    Ast::Rule(vec![(
                        prod.clone(),
                        cb.map_or(|| Span::new(0, 0), |g| g.item.span),
                    )])
                }
            },
            lrp::Span::new(toks[0].item.span.start, toks.last().unwrap().item.span.end),
